...
describe OpenTelemetry::Exporter::OTLP::Exporter do
  ...
  describe '#export' do
    ...
    it 'retries on timeout' do
      stub_request(:post, 'https://localhost:55681').to_timeout.then.to_return(status: 200)
      span_data = create_span_data
      result = exporter.export([span_data])
      _(result).must_equal(SUCCESS)
    end
    ...
    it 'returns TIMEOUT on timeout' do
      stub_request(:post, 'https://localhost:55681').to_return(status: 200)
      span_data = create_span_data
    ...
    it 'returns TIMEOUT on timeout after retrying' do
      stub_request(:post, 'https://localhost:55681').to_timeout.then.to_raise('this should not be reached')
      span_data = create_span_data
    ...
    it 'exports a span from a tracer' do
      stub_post = stub_request(:post, 'https://localhost:55681').to_return(status: 200)
    ...
    it 'compresses with gzip if enabled' do
      ...
      stub_post = stub_request(:post, 'https://localhost:55681').to_return do |request|
    ...
    it 'translates all the things' do
      ...
      assert_requested(:post, 'https://localhost:55681') do |req|
